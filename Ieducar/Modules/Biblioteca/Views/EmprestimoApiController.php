...
class EmprestimoApiController extends ApiCoreController
{
    ...
    {
        $objExemplar = new ExemplarEmprestimo();
        $exemplaresEmprestadosDoCliente = $objExemplar->lista(
        null,
        null,
        null,
        ...
        $objBiblioteca = new Biblioteca($this->getRequest()->biblioteca_id);
    ...
    protected function validatesSituacaoExemplarIsIn($expectedSituacoes)
    {
        if (! is_array($expectedSituacoes)) {
            $expectedSituacoes = [$expectedSituacoes];
        }
    ...
    */
    ...
    protected function loadQtdDiasEmprestimoForExemplar($exemplar = null)
    {
        ...
        $clienteTipoCliente = new ClienteTipoCliente();
        ...
        $clienteTipoExemplarTipo = new ClienteTipoExemplarTipo($clienteTipoId, $exemplarTipoId);
        ...
        if (! $clienteTipoExemplarTipo || ! is_numeric($clienteTipoExemplarTipo['dias_emprestimo'])) {
            throw new CoreExtensionException("Aparentemente não foi definido a quantidade de dias de emprestimo para o tipo de cliente '$clienteTipoId' e tipo de exemplar '$exemplarTipoId'.");
        }
    ...
    protected function getPendenciasForExemplar($exemplar)
    {
        if (! isset($exemplar['situacao'])) {
            throw new CoreExtensionException('Exemplar deve possuir uma chave \'situacao\' para getPendenciasForExemplar.');
        }
    ...
    {
        if (! $newSituacao) {
            throw new CoreExtensionException('$newSituacao não pode ser falso em updateSituacaoExemplar.');
        }
