...
//      $retorno = "Novo";
        ...
        if (is_numeric($this->cod_biblioteca)) {
            $obj = new Biblioteca($this->cod_biblioteca);
...
//      $this->campoRotulo( "bt_incluir_dia_semana", "Dia da Semana", "<a href='#' onclick=\"getElementById('incluir_dia_semana').value = 'S'; getElementById('tipoacao').value = ''; {$this->__nome}.submit();\"><img src='imagens/nvp_bot_incluir2.gif' title='Incluir' border=0></a>" );
    ...
    public function Editar()
    {
        ...
        if ($this->biblioteca_dia_semana) {
            $obj = new Biblioteca($this->cod_biblioteca, null, null, null, $this->valor_multa, $this->max_emprestimo, $this->valor_maximo_multa, null, null, $this->requisita_senha, 1, $this->dias_espera);
            $editou = $obj->edita();
            if ($editou) {
                //-----------------------EDITA DIA DA SEMANA------------------------//
                $obj  = new BibliotecaDia($this->cod_biblioteca);
                $excluiu = $obj->excluirTodos();
                if ($excluiu) {
                    foreach ($this->biblioteca_dia_semana as $campo) {
                        for ($i = 0; $i < sizeof($campo) ; $i++) {
                            $obj = new BibliotecaDia($this->cod_biblioteca, $campo[$i]);
                ...
                if ($excluiu) {
                    ...
                    if ($this->biblioteca_feriado) {
                        foreach ($this->biblioteca_feriado as $feriado) {
                            ...
                            $obj = new BibliotecaFeriados(null, $this->cod_biblioteca, $feriado['nm_feriado_'], null, $feriado['data_feriado_'], null, null, 1);
    ...
    public function Excluir()
    {
        ...
        $obj = new Biblioteca($this->cod_biblioteca, null, null, null, 'NULL', 'NULL', 'NULL', null, null, 'NULL', 1, 'NULL');
        $editou = $obj->edita();
        if ($editou) {
            $obj  = new BibliotecaDia($this->cod_biblioteca);
