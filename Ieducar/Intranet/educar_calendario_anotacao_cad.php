...
class indice extends clsCadastro
{
    ...
    public function Inicializar()
    {
        ...
        if (is_numeric($this->cod_calendario_anotacao)) {
            $obj = new CalendarioAnotacao($this->cod_calendario_anotacao);
    ...
    public function Novo()
    {
        ...
        $obj_dia = new CalendarioDia($this->ref_ref_cod_calendario_ano_letivo, $this->mes, $this->dia);
        if (!$obj_dia->existe()) {
            $obj_dia = new CalendarioDia($this->ref_ref_cod_calendario_ano_letivo, $this->mes, $this->dia, null, $this->pessoa_logada, null, null, null, null, 1);
            $ref_cod_dia_letivo = $obj_dia->cadastra();
            if (!$ref_cod_dia_letivo) {
                return false;
            }
        }
        $obj = new CalendarioAnotacao($this->cod_calendario_anotacao, $this->pessoa_logada, $this->pessoa_logada, $this->nm_anotacao, $this->descricao, $this->data_cadastro, $this->data_exclusao, $this->ativo);
        $this->cod_calendario_anotacao = $cadastrou = $obj->cadastra();
        if ($cadastrou) {
            $obj_anotacao_dia = new CalendarioDiaAnotacao($this->dia, $this->mes, $this->ref_ref_cod_calendario_ano_letivo, $cadastrou);
    ...
    public function Excluir()
    {
        ...
        $obj = new CalendarioAnotacao($this->cod_calendario_anotacao, $this->pessoa_logada, $this->pessoa_logada, $this->nm_anotacao, $this->descricao, $this->data_cadastro, $this->data_exclusao, 0);
        $excluiu = $obj->excluir();
        if ($excluiu) {
