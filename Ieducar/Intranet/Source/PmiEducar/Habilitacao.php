...
class Habilitacao extends Model
{
    ...
    public function __construct($cod_habilitacao = null, $ref_usuario_exc = null, $ref_usuario_cad = null, $nm_tipo = null, $descricao = null, $data_cadastro = null, $data_exclusao = null, $ativo = null, $ref_cod_instituicao = null)
    {
        $db = new Banco();
        $this->_schema = 'pmieducar.';
        $this->_tabela = "{$this->_schema}habilitacao";

        $this->_campos_lista = $this->_todos_campos = 'cod_habilitacao, ref_usuario_exc, ref_usuario_cad, nm_tipo, descricao, data_cadastro, data_exclusao, ativo, ref_cod_instituicao';
        ...
        if (is_numeric($cod_habilitacao)) {
            $this->cod_habilitacao = $cod_habilitacao;
        }
    ...
    {
        if (is_numeric($this->ref_usuario_cad) && is_string($this->nm_tipo) && is_string($this->ref_cod_instituicao)) {
            $db = new Banco();
            ...
            if (is_string($this->ref_cod_instituicao)) {
            ...
            return $db->InsertId("{$this->_tabela}_cod_habilitacao_seq");
        }
    ...
    public function edita()
    {
        if (is_numeric($this->cod_habilitacao) && is_numeric($this->ref_usuario_exc)) {
            ...
            if ($set) {
                $db->Consulta("UPDATE {$this->_tabela} SET $set WHERE cod_habilitacao = '{$this->cod_habilitacao}'");
    ...
    public function lista($int_cod_habilitacao = null, $int_ref_usuario_exc = null, $int_ref_usuario_cad = null, $str_nm_tipo = null, $str_descricao = null, $date_data_cadastro_ini = null, $date_data_cadastro_fim = null, $date_data_exclusao_ini = null, $date_data_exclusao_fim = null, $int_ativo = null, $int_ref_cod_instituicao = null)
    {
        ...
        if (is_numeric($int_cod_habilitacao)) {
            $filtros .= "{$whereAnd} cod_habilitacao = '{$int_cod_habilitacao}'";
            $whereAnd = ' AND ';
        }
    ...
    public function detalhe()
    {
        if (is_numeric($this->cod_habilitacao)) {
            $db = new Banco();
            $db->Consulta("SELECT {$this->_todos_campos} FROM {$this->_tabela} WHERE cod_habilitacao = '{$this->cod_habilitacao}'");
            $db->ProximoRegistro();
    ...
    public function existe()
    {
        if (is_numeric($this->cod_habilitacao)) {
            $db = new Banco();
            $db->Consulta("SELECT 1 FROM {$this->_tabela} WHERE cod_habilitacao = '{$this->cod_habilitacao}'");
            $db->ProximoRegistro();
