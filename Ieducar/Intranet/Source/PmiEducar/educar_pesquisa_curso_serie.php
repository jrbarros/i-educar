 ...
 * $obrigatorio = true;
 * $editar      = true;
 * Source("Source/pmieducar/educar_pesquisa_curso_serie.php");
 *
...
if ($privilegio == 4) {
    ...
    $opcoes = [ '' => 'Selecione' ];
    $objTemp = new EscolaSerie($permissoes->getEscola($this->pessoa_logada));
    $lista = $objTemp->lista($permissoes->getEscola($this->pessoa_logada));
    if (is_array($lista) && count($lista)) {
        foreach ($lista as $registro) {
            $obj_ser = new Serie($registro['ref_cod_serie']);
    ...
    include('Source/pmieducar/educar_pesquisa_instituicao_escola.php');
    $opcoes = [ '' => 'Selecione' ];
    $obj_escola = new Escola(null, null, null, $permissoes->getInstituicao($this->pessoa_logada), null, null, null, null, null, null, 1);
...
namespace iEducarLegacy\Intranet\Source\PmiEducar; $scripts_js = '
<script>
