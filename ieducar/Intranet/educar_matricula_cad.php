...
class indice extends clsCadastro
{
    ...
    public function Inicializar()
    {
        ...
        $obj_aluno = new Aluno($this->ref_cod_aluno);
    ...
    {
        return (int)(new Banco())->CampoUnico("
            select max_aluno
    ...
    public function Novo()
    {
        ...
        $this->ano = $_POST['ano'];
        $anoLetivoEmAndamentoEscola = new EscolaAnoLetivo();
        ...
        if (is_array($anoLetivoEmAndamentoEscola)) {
            ...
            if (is_array($m) && count($m) && !$dependencia) {
                ...
                if ($m['ref_ref_cod_serie'] == $this->ref_cod_serie) {
                    ...
                    $serie = new Serie($m['ref_ref_cod_serie'], null, null, $m['ref_cod_curso']);
                    $serie = $serie->detalhe();
                ...
                if (is_array($m) && count($m) && !$dependencia) {
                    ...
                    $cursoADeferir = new Curso($this->ref_cod_curso);
                    ...
                    if (($mesmoCursoAno || config('legacy.app.matricula.multiplas_matriculas') == 0) && !$cursoDeAtividadeComplementar) {
                        ...
                        require_once 'include/pessoa/Juridica.php';
                        ...
                        $escola = new Escola($m['ref_ref_cod_escola']);
                        ...
                        $curso = new Curso($m['ref_cod_curso']);
                        $curso = $curso->detalhe();
            ...
            $serie = new Serie($this->ref_cod_serie);
            ...
            if ($verificarDataCorte && !$reload) {
                $instituicao = new Instituicao($this->ref_cod_instituicao);
                ...
                $objAluno = new Aluno($this->ref_cod_aluno);
                $detAluno = $objAluno->detalhe();

                $objPes = new PessoaFisica($detAluno['ref_idpes']);
            ...
            $objSerie = new Serie($this->ref_cod_serie);
            ...
            $objInstituicao = new Instituicao($this->ref_cod_instituicao);
            ...
            if ($controlaEspacoUtilizacaoAluno) {
                $objTurma = new Turma($this->ref_cod_turma);
            ...
            $obj_matricula_aluno = new Matricula();
            ...
            $obj_CandidatoReservaVaga = new CandidatoReservaVaga();
            ...
            if ($cadastrou) {
                if ($countEscolasIguais > 0) {
                    $obj_crv = new CandidatoReservaVaga($this->ref_cod_candidato_reserva_vaga);
                    $obj_crv->vinculaMatricula($this->ref_cod_escola, $this->cod_matricula, $this->ref_cod_aluno);
                }

                if ($this->ref_cod_candidato_fila_unica) {
                    $obj_cfu = new CandidatoFilaUnica($this->ref_cod_candidato_fila_unica);
    ...
    {
        $aluno = new Aluno($this->ref_cod_aluno);
        $aluno = $aluno->detalhe();

        $pessoa = new PessoaFisica($aluno['ref_idpes']);
    ...
    public function permiteMatriculaSerieDestino()
    {
        ...
        $objSequenciaSerie = new SequenciaSerie;
    ...
    public function Excluir()
    {
        ...
        $obj_sequencia = new SequenciaSerie();
    ...
    {
        $enturmacaoExists = new MatriculaTurma();
    ...
    {
        $obj_t = new Turma($this->ref_cod_turma);
        $det_t = $obj_t->detalhe();
