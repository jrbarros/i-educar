...
$pessoa_logada = Session::get('id_pessoa');
    ...
    elseif ($nivel_usuario != 1) {
        $obj_usuario = new Usuario($pessoa_logada);
    ...
    if ($get_curso) {
        ...
        if ($this->ref_cod_escola) {
            $obj_escola_curso = new EscolaCurso();
            ...
            if ($qtd_bibliotecas == 1 && ($nivel_usuario == 4 || $nivel_usuario == 8)) {
                ...
                if (is_array($ref_cod_biblioteca_) && count($ref_cod_biblioteca_)) {
                    foreach ($ref_cod_biblioteca_ as $biblioteca) {
                        $obj_biblioteca = new Biblioteca($biblioteca['ref_cod_biblioteca']);
...
namespace iEducarLegacy\Intranet\Source\PmiEducar; if ($get_cursos_nao_padrao) {?>
                xml.envia( "educar_curso_xml.php?esc="+campoEscola+"&padrao_ano_escolar=nao" );
            <?php

namespace iEducarLegacy\Intranet\Source\PmiEducar; } else {?>
