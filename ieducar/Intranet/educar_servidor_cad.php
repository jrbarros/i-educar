...
class indice extends clsCadastro
{
            ...
            if ($registro) {
                ...
                if ($lst_servidor_disciplina) {
                    foreach ($lst_servidor_disciplina as $disciplina) {
                        $obj_disciplina = new Disciplina($disciplina['ref_cod_disciplina']);
    ...
    public function Gerar()
    {
        ...
        $objTemp = new CadastroEscolaridade();
        $lista = $objTemp->lista();
...
JS;
    ...
    public function Novo()
    {
        ...
        $obj   = new Servidor($this->cod_servidor, null, null, null, null, null, null, $this->ref_cod_instituicao);

        if ($obj->detalhe()) {
            $this->carga_horaria = str_replace(',', '.', $this->carga_horaria);
            $obj = new Servidor($this->cod_servidor, null, $this->ref_idesco, $this->carga_horaria, null, null, 1, $this->ref_cod_instituicao);
            ...
            $obj_2 = new Servidor($this->cod_servidor, null, $this->ref_idesco, $this->carga_horaria, null, null, 1, $this->ref_cod_instituicao);
    ...
    public function Editar()
    {
        ...
        if ($this->ref_cod_instituicao == $this->ref_cod_instituicao_original) {
            ...
            $this->carga_horaria = str_replace(',', '.', $this->carga_horaria);
            $obj_quadro_horario = new QuadroHorarioHorarios(
                null,
                null,
                null,
                null,
    ...
    public function cadastraFuncoes()
    {
        ...
        if ($this->ref_cod_funcao) {
            foreach ($this->ref_cod_funcao as $k => $funcao) {
                ...
                $obj_servidor_funcao = new ServidorFuncao(null, null, null, null, $cod_servidor_funcao);
                ...
                if (empty($cod_servidor_funcao)) {
                    $obj_servidor_funcao = new ServidorFuncao($this->ref_cod_instituicao, $this->cod_servidor, $funcao);
        ...
        if ($existe_funcao_professor) {
            ...
            if ($cursos_servidor) {
                ...
                foreach ($cursos_servidor as $curso) {
                    $obj_curso_servidor = new ServidorCursoMinistra($curso, $this->ref_cod_instituicao, $this->cod_servidor);
    ...
    {
        $obj_servidor_funcao = new ServidorFuncao($this->ref_cod_instituicao, $this->cod_servidor);
    ...
    {
        $obj_servidor_funcao = new ServidorFuncao($this->ref_cod_instituicao, $this->cod_servidor, $funcao, $matricula);
        $obj_servidor_funcao->cadastra();
    }

    public function excluiDisciplinas()
    {
        $obj_servidor_disciplina = new ServidorDisciplina(null, $this->ref_cod_instituicao, $this->cod_servidor);
        $obj_servidor_disciplina->excluirTodos();
    }

    public function excluiCursos()
    {
        $obj_servidor_curso = new ServidorCursoMinistra(null, $this->ref_cod_instituicao, $this->cod_servidor);
    ...
    protected function createOrUpdateDeficiencias()
    {
        ...
        foreach ($this->getRequest()->deficiencias as $id) {
            if (!empty($id)) {
                $deficiencia = new CadastroFisicaDeficiencia($servidorId, $id);
