...
function horasUteisInterval($data_inicial, $data_final)
{
    ...
    if ($data_inicial == $data_final) {
        $horas[ $data_inicial ][] = ['ini'=> $hora_inicial, 'fim'=>$hora_final];
    } else {
        $horas[ $data_inicial ][] =     ['ini'=> $hora_inicial, 'fim'=>'18:00'];

        while ($data_inicial != $data_final) {
            $temp = explode('/', $data_inicial);
            $data_inicial = date('Y/m/d', mktime(0, 0, 0, $temp[1], $temp[2]+1, $temp[0]));
            if ($data_inicial == $data_final) {
                $horas[ $data_inicial ][] = ['ini'=> '08:00', 'fim'=>$hora_final];
            } else {
                $horas[ $data_inicial ][] = ['ini'=> '08:00', 'fim'=>'18:00'];
            }
        }
    }
...
function horasInterval($data_inicial, $data_final)
{
    ...
    if ($data_inicial == $data_final) {
        ...
        $horas[ $data_inicial ][] =     ['ini'=> $hora_inicial, 'fim'=>'23:59:59'];

        while ($data_inicial != $data_final) {
            $temp = explode('/', $data_inicial);
            $data_inicial = date('Y/m/d', mktime(0, 0, 0, $temp[1], $temp[2]+1, $temp[0]));
            if ($data_inicial == $data_final) {
                $horas[ $data_inicial ][] = ['ini'=> '00:00', 'fim'=>$hora_final];
            } else {
                $horas[ $data_inicial ][] = ['ini'=> '00:00', 'fim'=>'23:59:59'];
            }
        }
    }
