...
class TipoRegime extends Model
{
    ...
    public function __construct($cod_tipo_regime = null, $ref_usuario_exc = null, $ref_usuario_cad = null, $nm_tipo = null, $data_cadastro = null, $data_exclusao = null, $ativo = null, $ref_cod_instituicao = null)
    {
        $db = new Banco();
        $this->_schema = 'pmieducar.';
        $this->_tabela = "{$this->_schema}tipo_regime";
        ...
        if (is_numeric($cod_tipo_regime)) {
            $this->cod_tipo_regime = $cod_tipo_regime;
        }
    ...
    public function cadastra()
    {
        if (is_numeric($this->ref_usuario_cad) && is_string($this->nm_tipo) && is_numeric($this->ref_cod_instituicao)) {
            ...
            return $db->InsertId("{$this->_tabela}_cod_tipo_regime_seq");
        }
    ...
    public function edita()
    {
        if (is_numeric($this->cod_tipo_regime) && is_numeric($this->ref_usuario_exc)) {
            ...
            if ($set) {
                $db->Consulta("UPDATE {$this->_tabela} SET $set WHERE cod_tipo_regime = '{$this->cod_tipo_regime}'");
    ...
    public function lista($int_cod_tipo_regime = null, $int_ref_usuario_exc = null, $int_ref_usuario_cad = null, $str_nm_tipo = null, $date_data_cadastro_ini = null, $date_data_cadastro_fim = null, $date_data_exclusao_ini = null, $date_data_exclusao_fim = null, $int_ativo = null, $int_ref_cod_instituicao = null)
    {
        ...
        if (is_numeric($int_cod_tipo_regime)) {
            $filtros .= "{$whereAnd} cod_tipo_regime = '{$int_cod_tipo_regime}'";
            $whereAnd = ' AND ';
        }
    ...
    public function detalhe()
    {
        if (is_numeric($this->cod_tipo_regime)) {
            $db = new Banco();
            $db->Consulta("SELECT {$this->_todos_campos} FROM {$this->_tabela} WHERE cod_tipo_regime = '{$this->cod_tipo_regime}'");
            $db->ProximoRegistro();
    ...
    public function existe()
    {
        if (is_numeric($this->cod_tipo_regime)) {
            $db = new Banco();
            $db->Consulta("SELECT 1 FROM {$this->_tabela} WHERE cod_tipo_regime = '{$this->cod_tipo_regime}'");
            $db->ProximoRegistro();
