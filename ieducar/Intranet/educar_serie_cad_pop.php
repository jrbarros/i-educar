...
//      $this->disciplina_serie = unserialize( urldecode( $this->disciplina_serie ) );
        if ($this->disciplinas) {
            $obj = new Serie(null, null, $this->pessoa_logada, $this->ref_cod_curso, $this->nm_serie, $this->etapa_curso, $this->concluinte, $this->carga_horaria, null, null, 1, $this->idade_inicial, $this->idade_final, $this->media_especial);
            $cadastrou = $obj->cadastra();
            if ($cadastrou) {
                $elemento = ($_GET['ref_ref_cod_serie']) ? 'ref_ref_cod_serie' : 'ref_cod_serie';
                //-----------------------CADASTRA DISCIPLINA------------------------//
                foreach ($this->disciplinas as $disciplina) {
                    $obj = new DisciplinaSerie($disciplina, $cadastrou);
