...
class indice extends clsCadastro
{
    ...
    public function Inicializar()
    {
        ...
        if (is_numeric($this->cod_emprestimo)) {
            $obj = new ExemplarEmprestimo($this->cod_emprestimo);
    ...
    public function Gerar()
    {
        ...
        $obj_cliente_suspenso = new Cliente();
...
//      echo "<pre>";print_r($lst_cliente_suspenso);
        ...
        $obj_exemplar_emprestimo = new ExemplarEmprestimo();
        ...
        // foreign keys
        $obj_cliente = new Cliente($this->ref_cod_cliente);
...
//                          $lst_exemplar = $obj_exemplar->lista($exemplar,null,null,null,null,null,null,2,null,null,null,null,null,1,null,null,null,null,$this->ref_cod_biblioteca);
                            ...
                            if (is_array($lst_exemplar) && count($lst_exemplar)) {
                                ...
                                $obj_situacao = new Situacao($cod_situacao);
...
//                          echo "<pre>"; print_r($det_situacao); die();
                                ...
                                if ($situacao_padrao == 1 && $permite_emprestimo == 2) {
                                    ...
                                    if (is_array($lst_reservas) && count($lst_reservas)) {
                                        ...
                                        $obj = new Reservas($reservas['cod_reserva'], $this->pessoa_logada, null, null, null, null, date('Y-m-d'), null, 0);
                                        $editou = $obj->edita();
                                        if ($editou) {
                                            // adiciona exemplar para empréstimo
                                            $obj_exemplar = new Exemplar($exemplar);
                                            ...
                                            if (is_array($reservas) && count($reservas)) {
                                                ...
                                                if ($data_disponivel < date('Y-m-d')) {
                                                    ...
                                                    if (is_array($lst) && count($lst)) {
                                                        ...
                                                        $obj = new Reservas($cod_reserva, $this->pessoa_logada, null, null, null, null, null, null, 0);
                                                        $excluiu = $obj->excluir();
                                                        if ($excluiu) {
                                                            // adiciona exemplar para empréstimo
                                                            $obj_exemplar = new Exemplar($exemplar);
    ...
    public function Novo()
    {
        ...
        if ($this->exemplar_emprestimo) {
            ...
            foreach ($this->exemplar_emprestimo as $campo) {
                $obj = new ExemplarEmprestimo(null, null, $this->pessoa_logada, $this->ref_cod_cliente, $campo);
                ...
                if ($cadastrou) {
                    ...
                    if (is_array($lst_situacao) && count($lst_situacao)) {
                        ...
                        $obj = new Exemplar($campo, null, null, null, $cod_situacao, $this->pessoa_logada, null, null, null, null, null, 1);
                        $editou = $obj->edita();
                        if (!$editou) {
